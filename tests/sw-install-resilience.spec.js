...
test('初回installでasset manifestが503でもcore shellがactivateしoffline再起動できる', async ({ browser }) => {
  ...
  const cacheState = await page.evaluate(async () => {
    ...
    const coreName = names.find(name => name === 'tabenai-to-shinu-core-4.6.0');
  ...
  expect(cacheState.names).toContain('tabenai-to-shinu-core-4.6.0');
...
test('presentation precacheのSVG 1枚が404でもcoreをactivateし404を保存しない', async ({ browser }) => {
  ...
  expect(cacheState.names).toContain('tabenai-to-shinu-core-4.6.0');
  expect(cacheState.names).toContain('tabenai-to-shinu-presentation-4.6.0');
